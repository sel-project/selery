...
mixin("alias Games = TypeTuple!(" ~ (){ string[] ret;foreach(g,pr;["pocket":supportedPocketProtocols.keys,"java":supportedJavaProtocols.keys]){foreach(p;pr){ret~="\""~g~p.to!string~"\"";}}return ret.join(","); }() ~ ");");
