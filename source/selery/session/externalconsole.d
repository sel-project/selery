 ...
 * 
 */
module selery.session.externalconsole;
...
import selery.network.socket : BlockingSocket;
import selery.path : Paths;
import selery.session.http : Request, Response;
...
abstract class ExternalConsoleSession : Session {
	...
	public shared this(shared HubServer server, shared Socket socket) {
		super(server);
...
class ClassicExternalConsoleSession : ExternalConsoleSession {
	...
	public shared this(shared HubServer server, shared Socket sharedSocket, bool delegate(ubyte[], ubyte[]) auth) {
